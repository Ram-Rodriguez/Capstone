              ...
              <table id="example1" class="table table-bordered table-striped">
                ...
                @forelse($appointments as $item)
                  <tr>
                    ...
                    <td>{{ $item->title }}</td>
                    <td><a href="{{ route('head.appointments.edit', $item->id) }}" class="btn btn-primary">Edit</a></td>
                    <form action="{{ route('head.appointments.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this record?')">
                    @csrf
                    @method('PUT')
